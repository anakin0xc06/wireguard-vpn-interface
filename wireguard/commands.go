...
func cmdAddDevLink(_inteface, _type string) *exec.Cmd {
...
func cmdSetDevMTU(_interface string, mtu int) *exec.Cmd {
	return exec.Command("sh", "-c", fmt.Sprintf("ip link set mtu %d up dev %s", mtu, _interface))
}

func cmdAddDevAddressIPv4(_interface, addr string) *exec.Cmd {
	return exec.Command("sh", "-c", fmt.Sprintf("ip -4 address add %s dev %s", addr, _interface))
}

func cmdAddDevAdressIPv6(_interface, addr string) *exec.Cmd {
